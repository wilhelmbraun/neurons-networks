...
class Neuron:
    def __init__(self,init_state = [],method="FE",sigma=0.):
	self.sigma=sigma
	if method=="FE" and sigma==0:
	    self.integrate=self.forwardEuler
	elif method=="FE" and sigma > 0:
	    self.integrate=self.noisyEuler
	    
	self.time_elapsed = 0   
	self.state = np.asfarray(init_state)
    
    def forwardEuler(self,u,f,dt,sigma=0.):
	return u+f*dt

    def noisyEuler(self,u,f,dt,sigma):
	h=u+f*dt
	h[0]=h[0]+np.sqrt(dt)*sigma*rnd.normal(0.,1.,1)
	return h
    
    def timestep(self, dt):
	self.state=self.integrate(self.state, self.RHS, dt, self.sigma)
...
class O_Rotstein(Neuron):
    ...
    def __init__(self,init_state,method="FE",sigma=0.):
        Neuron.__init__(self,init_state,method,sigma)
    ...
    def getIh(self):
	return self.params[5]*(0.65*self.state[5]+0.35*self.state[6])*(self.params[5]-self.state[0])
...
class I_Rotstein(Neuron):
    ...
    def buildRHS(self, I):
        ...
        [C,g_L, g_Na, g_K, g_h, v_Na, v_K, v_h, v_L, tau_R, tau_D] = self.params
